...
#include "sqlite3.h"
#include "SQL.h"
#include "CISQLite3/SQLiteBlueprintNodes.h"
#include "SQLiteDatabaseStructs.h"
#include "SQLiteDatabase.generated.h"

USTRUCT(BlueprintType)
struct DS_SQL_API FSQLiteDatabaseReference
{
...
*/
UCLASS()
class DS_SQL_API USQLiteDatabase : public UObject, public ISQL
{
...
public:
	...
	FSQLQueryResult GetData(const FString& DatabaseName, const FString& Query);
    ...
    FSQLQueryResult GetDataBP(const FSQLiteDatabaseReference& DataSource, TArray<FString> Fields, FSQLiteQueryFinalizedQuery Query, int32 MaxResults = -1, int32 ResultOffset = 0);
...
private:
	...
	SQLiteQueryResult RunQueryAndGetResults(FString DatabaseName, FString Query);
	...
	void PrepareStatement(const FString Query, sqlite3** Db, int32** SqlReturnCode,	sqlite3_stmt** PreparedStatement);
